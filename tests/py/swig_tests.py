import my_string_swig as ms
import sys
#basic c-tor
basic_ctor=ms.MyString()
assert(str(basic_ctor)=="")
#string ctor
str_ctor=ms.MyString("lorem ipsum")
assert(str(str_ctor)=="lorem ipsum")
#n char ctor
n_str_ctor=ms.MyString("lorem ipsum",5)
assert(str(n_str_ctor)=="lorem")
#copy ctor
copy_ctor=ms.MyString(n_str_ctor)
...
#indexing (does not work smh)
#str_ctor[0]="0"
#assert(str(str_ctor)=="0orem ipsum")
#assert(str_ctor[0]=="0")
#operator testing
less=ms.MyString("abc little")
bigger=ms.MyString("def large")
assert(less<bigger)
assert(bigger>less)
assert(bigger!=less)
assert(less<=bigger)
assert(bigger>=less)
assert(bigger==ms.MyString(bigger))
