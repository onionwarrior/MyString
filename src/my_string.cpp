...
//Default c-tor
MyString::MyString() : str_{new char[capacity_]()}
{
}
//std::string c-tor
MyString::MyString(const std::string &arg) : MyString{arg.c_str()}
{
}
//std::initializer_list c-tor
MyString::MyString(std::initializer_list<char> init_list) : size_{init_list.size()},
                                                            capacity_{size_ + 1},
...
MyString::MyString(const char *c_str, size_t count) : size_{c_str ? (count == default_append) ? std::strlen(c_str) : count : 0},
                                                      capacity_{size_ + 1},
                                                      str_{c_str ? new char[capacity_]() : nullptr}
{
    if (count == default_append)
        count = size_;
    if (count > size_)
    {
        delete str_;
        throw std::out_of_range("count of chars was more that str's length");
    }
    if (nullptr == c_str)
        throw std::invalid_argument("nullptr passed to constructor");
    std::memcpy(str_, c_str, count);
}
//count char constructor
MyString::MyString(char c, size_t count) : size_{count},
                                           capacity_{size_ + 1},
                                           str_{new char[capacity_]()}
{
...
//Copy constructor
MyString::MyString(const MyString &copy) : MyString{copy.c_str()}
{
}
//Move constructor
MyString::MyString(MyString &&rhs) : size_{std::exchange(rhs.size_, 0)},
